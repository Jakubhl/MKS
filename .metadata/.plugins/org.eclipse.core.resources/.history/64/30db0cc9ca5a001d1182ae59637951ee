...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		static int16_t DS18B20_hodnota;
		/*
		 uint32_t cas1;
		 cas1 = HAL_GetTick();
		 if (HAL_GetTick() - cas1 >= 750) {
		...
		uint8_t tl1 = HAL_GPIO_ReadPin(S1_GPIO_Port, S1_Pin);
		uint8_t tl2 = HAL_GPIO_ReadPin(S2_GPIO_Port, S2_Pin);
		//static uint32_t button_time;

		//if(tl2==1){state = SHOW_POT;}else{state = SHOW_TEMP;};

		//uint32_t cas2;
		//cas2 = HAL_GetTick();
		/*if (HAL_GetTick() - cas1 >= 100) {
		 OWConvertAll();*/

		if (HAL_GetTick() >= lastDisplayTicks + 100) {



			uint16_t adc;
			adc = HAL_ADC_GetValue(&hadc);
			int16_t NTC_hodnota = NTC_TABULKA_HODNOT[adc];
			...
			if (state == SHOW_DS18B20) {
				sct_value(DS18B20_hodnota / 10, 0);
			}

			if (state == SHOW_NTC) {
				sct_value(NTC_hodnota, 0);
			}
		...
		if (HAL_GetTick() >= lastMeasurementTicks + 750) {
			OWReadTemperature(&DS18B20_hodnota);
			OWConvertAll();
